...
"""右側突破策略
  ...
  ======================================================================
  一、 核心防禦與大盤濾網 (Market Guardrail)
  ======================================================================
  1. 大盤年線過濾：若 `market_above_ma240` 為 False（大盤在年線 MA240 之下），策略直接不予觸發。
  2. 資料長度檢查：確保資料筆數至少 120 筆，避免因資料不足導致計算錯誤。

  ======================================================================
  二、 技術指標計算清單 (Technical Indicators)
  ======================================================================
  - 移動平均線：MA5、MA10、MA20、MA60 (季線)、MA240 (年線)
  - 量能指標：20 日均量 (`Vol_MA20`)
  - 價格區間：前 1 日起算的 20 日最高價 (`Max_20`) 與最低價 (`Min_20`)
  - 乖離率與振幅：
    * 20日均線乖離率 (`Bias_20`) = (收盤價 - MA20) / MA20
    * 20日區間振幅 (`Range_20`) = (20日最高 - 20日最低) / MA20
  - 均線斜率：MA5、MA20、MA60 的滾動斜率

  ======================================================================
  三、 核心觸發條件與濾網 (Core Conditions)
  ======================================================================
  【條件一：箱型突破與量能爆發防禦版】 (用於結構篩選與勝率提升)
    - 突破防禦：收盤價突破過去 20 日高點 (`close > max_20`)
    - 量能爆發：成交量放大至 20 日均量的 1.5 倍以上 (`volume >= Vol_MA20 * 1.5`)
    - 多頭排列：短中期均線呈多頭排列 (`MA5 > MA10 > MA20 > MA60`)
    - 均線揚升：MA20 與 MA60 斜率向上或持平 (`>= 0.0`)
    - 年線過濾：若資料滿 240 日，收盤需在年線之上
    - 乖離安全：20日乖離率控制在 12% 以內 (`Bias_20 <= 0.12`)
    - 盤整緊密：20日區間振幅小於 25% (`Range_20 <= 0.25`)

  【條件二：短線強勢動能與均線交叉發散版】 (策略主要觸發核心)
    - 價格站上：收盤價高於 5 日線 (`close > MA5`)
    - 黃金交叉：MA5 向上黃金交叉 MA20 (`MA5 > MA20` 且昨日 `MA5 <= MA20`)
    - 動能強勢：5日線斜率強勢向上 (`MA5_slope >= 0.005`)
    - 短線多頭：短天期均線發散 (`MA5 > MA10 > MA20`)
    - MA20保護：MA20 斜率未顯著下彎 (`MA20_slope >= -0.001`)
    - 乖離控制：20日乖離率在 10% 以內 (`Bias_20 <= 0.10`)

  ======================================================================
  四、 選股分類與輸出邏輯 (Classification & Output)
  ======================================================================
  - 第一型 (is_type_1)：符合「條件二」之短線強勢動能與均線交叉。
  - 第二型 (is_type_2)：同時符合「條件二」與「條件一」，屬於勝率極高之結構突破。
  - 觸發判定 (is_hit)：只要符合條件二，即視為策略觸發訊號。
  - 停損參考：以近 10 日最低收盤價作為建議停損參考價 (`recent_low`)。
  ======================================================================
  """
