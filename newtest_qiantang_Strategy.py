...
"""
錢塘潮選股系統 - 多方選股策略指定天期區間歷史掃描 (含完整未來前瞻與極值績效追蹤)
說明：對齊 test_Monitor.py 逐日推進與交易日過濾架構，執行錢塘潮 7 大多方選股公式，
     並自動計算觸發後 T+3/T+5/T+10/T+20 報酬率與未來 1 年內最高/最低極值報酬，匯出至 Multi-Sheet Excel 儀表板。
"""
...
from utils import (
    ...
    parse_stock_ids,
    get_fm_trading_days,
    calculate_forward_horizon_returns, # 前瞻固定天數報酬 (T+3, T+5, T+10, T+20)
...
def scan_qiantang_strategy_day(
    ...
    """單日個股選股掃描核心，並於觸發時計算前瞻 (T+3/5/10/20) 與極值績效"""
    day_hits = []
...
def run_qiantang_strategy_range_scan(
    ...
    # 2. 準備歷史資料抓取區間 (結束日期往後延伸 365 天以計算未來的極值與區間報酬)
    ...
    # 3. 全域指標預處理 (一次計算全歷史輕鬆線、KD、20日均量等)
